...
/**
 * Based on the IpcWrapper from companion, adapted for satellite (no ejson).
 */
...
export class IpcWrapper<TOutbound extends { [key: string]: any }, TInbound extends { [key: string]: any }> {
	...
	async sendWithCb<T extends keyof TOutbound>(
		...
		} = Promise.withResolvers<ReturnType<TOutbound[T]>>()
		const callbacks: PendingCallback = { timeout: undefined, resolve: resolvePromise, reject: rejectPromise }
