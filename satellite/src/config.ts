...
export const satelliteConfigSchema: Schema<SatelliteConfig> = {
	...
	surfacePluginsEnabled: {
		...
		default: {
			'203-systems-mystrix': false,
			'blackmagic-controller': false,
			'blackmagic-videohub-panel': false,
			'contour-shuttle': false,
			'elgato-stream-deck': true,
			'framework-macropad': false,
			'idisplay-infinitton': true,
			'logitech-mx-creative-console': false,
			loupedeck: true,
			'mirabox-stream-dock': false,
			'vec-footpedal': false,
			'xencelabs-quick-keys': false,
			xkeys: false,
		},
	},
}
