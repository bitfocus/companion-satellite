; Custom NSIS installer hooks for Bitfocus Companion Satellite.
;
; This file is referenced explicitly via `nsis.include` in the `nsis` block of
; tools/build_electron.mts. It defines the `customInstall` / `customUnInstall`
...
; runtime. Companion Satellite's network services run inside the bundled
; `node.exe` processes, not the Satellite launcher, so without these rules
; Windows pops a per-binary firewall prompt the first time a service binds to a
...
!macro companionAddFirewallRules
  ...
  companionFwAddLoop:
    ...
    IfFileExists "$0\$2\node.exe" 0 companionFwAddNext
      ...
      nsExec::Exec 'netsh advfirewall firewall delete rule name="Bitfocus Companion Satellite ($2)"'
      Pop $3
      nsExec::Exec 'netsh advfirewall firewall add rule name="Bitfocus Companion Satellite ($2)" dir=in action=allow program="$0\$2\node.exe" enable=yes profile=domain,private,public'
      Pop $3
...
!macro customInstall
  ...
  Pop $R0
  ${If} $R0 == "Admin"
    DetailPrint "Configuring Windows Firewall rules for Companion Satellite..."
...
!macro customUnInstall
  ...
  Pop $R0
  ${If} $R0 == "Admin"
    DetailPrint "Removing Companion Satellite Windows Firewall rules..."
