#!/bin/bash
#
# Companion Satellite desktop installer for Linux.
#
# Copies this Companion Satellite build to a proper install location (system-wide
# or per-user), registers it in the application menu with an icon, installs the
# udev rules needed to access USB surfaces, and checks for the system libraries
# it needs.
#
# Usage:
...
APP_ID="companion-satellite"
LAUNCHER="companion-satellite"
# Desktop builds use the uaccess rules, which grant the logged-in user access to
# the USB surfaces automatically - no group membership or re-login required.
UDEV_RULES="50-satellite-desktop.rules"
...
usage() {
	cat <<EOF
Companion Satellite installer

Usage: $0 [options]

Options:
  --system    Install system-wide to /opt/$APP_ID (requires root)
  --user      Install for the current user to ~/.local/share/$APP_ID
...
# Sanity check: make sure we're being run from inside a Companion Satellite build.
if [ ! -x "$SOURCE_DIR/$LAUNCHER" ]; then
	echo "Error: $LAUNCHER was not found next to this script." >&2
	echo "Run install.sh from inside the extracted Companion Satellite folder." >&2
	exit 1
fi
...
if [ -z "$SCOPE" ]; then
	...
	echo "Where should Companion Satellite be installed?"
	echo "  1) System-wide (/opt/$APP_ID, needs administrator password)"
	echo "  2) Just for me (~/.local/share/$APP_ID, no password)"
	while true; do
...
ICON_DIR="$HICOLOR_DIR/512x512/apps"
DESKTOP_FILE="$APPLICATIONS_DIR/$APP_ID.desktop"

# Don't let the install target be the folder we're running from.
if [ "$SOURCE_DIR" = "$INSTALL_DIR" ]; then
	echo "Error: Companion Satellite is already located at the install target ($INSTALL_DIR)." >&2
...
echo "Installing Companion Satellite to $INSTALL_DIR..."
mkdir -p "$INSTALL_DIR"
...
sed -e "s|@EXEC_PATH@|$INSTALL_DIR/$LAUNCHER|g" \
...
# Run a block of commands as root: directly if we already are, otherwise via
# sudo. Returns non-zero (without aborting the installer) if root can't be had,
# e.g. the user cancels the sudo prompt.
run_as_root() {
	if [ "${EUID:-$(id -u)}" -eq 0 ]; then
		...
		echo "Error: this step needs root but sudo is not available." >&2
		return 1
	fi
}

# Install the udev rules so the logged-in user can talk to USB surfaces. This
# needs root; if the user declines the sudo prompt we carry on without it.
install_udev_rules() {
	local rules_src="$SOURCE_DIR/$UDEV_RULES"
	if [ ! -f "$rules_src" ]; then
		echo "Note: $UDEV_RULES not found next to the installer; skipping udev setup."
		return
	fi

	echo
	echo "Companion Satellite needs udev rules so it can access USB surfaces"
	echo "(Stream Deck, Loupedeck, X-keys and similar). Installing these needs"
	echo "administrator rights."

	if run_as_root <<EOF
set -e
# Install the rules and ask udev to pick them up.
install -D -m 0644 "$rules_src" "/etc/udev/rules.d/$UDEV_RULES"
udevadm control --reload-rules || true
udevadm trigger || true
EOF
	then
		echo "udev rules installed. Replug your USB surfaces if they were already connected."
	else
		echo "Warning: udev rules were not installed (cancelled or insufficient rights)."
		echo "         USB surfaces may be inaccessible until you install them manually -"
		echo "         see the README next to this installer for the manual steps."
	fi
}
install_udev_rules

# Best-effort check for the system libraries Companion Satellite needs.
check_dependencies() {
	local pkgs="libusb-1.0-0 libudev1 libfontconfig1"
	if ! command -v dpkg >/dev/null 2>&1; then
		echo "Note: Companion Satellite needs these system libraries to be present:"
...
echo
echo "Companion Satellite is installed at $INSTALL_DIR"
echo "It should now appear in your application menu."
echo "You can also launch it directly with: $INSTALL_DIR/$LAUNCHER"
echo
