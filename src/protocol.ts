/*
 * This file is part of the Companion project
 * Copyright (c) 2019 Bitfocus AS
 * Authors: Håkon Nessjøen <haakon@bitfocus.io>, William Viker <william@bitfocus.io>
 *
 * This program is free software.
 * You should have received a copy of the MIT licence as well as the Bitfocus
 * Individual Contributor License Agreement for companion along with
 * this program.
 *
 * You can be released from the requirements of the license by purchasing
 * a commercial license. Buying such a license is mandatory as soon as you
 * develop commercial activities involving the Companion software without
 * disclosing the source code of your own applications.
 *
 */

/// <reference types="./binopsy" />
import Parser = require('binopsy')
import { Socket } from 'net'

// prettier-ignore
const crc8array = [
	
];

export interface ProtocolHeader {
	signature: number
	command: number
	length: number
}

export class Protocol {
	public static readonly SUPPORTED_MAJOR = 1
	public static readonly SUPPORTED_MINIOR = 1

	public static readonly SA_HEADER_PARSER = new Parser()
		.endianess('big')
		.uint16('signature')
		.uint8('command')
		.uint16('length')

	public static readonly SCMD_ADDDEVICE_PARSER = new Parser()
		.endianess('big')
		.string('serialNumber', { length: 20 })
		.uint16('deviceId')

	public static readonly SCMD_ADDDEVICE2_PARSER = new Parser()
		.endianess('big')
		.string('serialNumber', { length: 20 })
		.uint16('deviceId')
		.uint8('keysTotal')
		.uint8('keysPerRow')

	public static readonly SCMD_REMOVEDEVICE_PARSER = new Parser().endianess('big').uint16('deviceId')

	public static readonly SCMD_VERSION_PARSER = new Parser()
		.endianess('big')
		.uint8('versionMajor')
		.uint8('versionMinior')

	public static readonly SCMD_DRAW7272_PARSER = new Parser()
		.endianess('big')
		.uint16('deviceId')
		.uint8('keyIndex')
		.buffer('image', { length: 15552 })

	public static readonly SCMD_BUTTON_PARSER = new Parser()
		.endianess('big')
		.uint16('deviceId')
		.uint8('keyIndex')
		.uint8('state')

	public static readonly SCMD_BRIGHTNESS_PARSER = new Parser().endianess('big').uint16('deviceId').uint8('percent')

	public static readonly SCMD_PING = 0x01
	public static readonly SCMD_PONG = 0x02
	public static readonly SCMD_VERSION = 0x03
	public static readonly SCMD_ADDDEVICE = 0x10
	public static readonly SCMD_REMOVEDEVICE = 0x11
	public static readonly SCMD_ADDDEVICE2 = 0x12
	public static readonly SCMD_BUTTON = 0x20
	public static readonly SCMD_DRAW7272 = 0x30
	public static readonly SCMD_BRIGHTNESS = 0x31

	public static calcCRC(buffer: Buffer): number {
		let crc = 0

		for (let i = 0; i < buffer.length; ++i) {
			crc = crc8array[crc ^ buffer[i]]
		}

		return crc
	}

	public static readHeader(buffer: Buffer): ProtocolHeader | false | -1 {
		// Not enough data
		if (buffer.length < 5) return false

		if (buffer.readUInt16BE(0) !== 0x5a7e) {
			// SATE(lite)
			return -1 // out of sync
		}

		return this.SA_HEADER_PARSER.parse(buffer)
	}

	public static sendPacket(socket: Socket, command: number, data: Buffer | undefined): void {
		if (data === undefined) {
			data = Buffer.alloc(0)
		}

		// if (typeof data !== 'object' || !(data instanceof Buffer)) {
		// 	data = Buffer.from(data)
		// }

		const buffer = Buffer.alloc(5 + data.length + 1)

		const header: ProtocolHeader = {
			signature: 0x5a7e,
			command: command,
			length: data.length,
		}

		this.SA_HEADER_PARSER.serialize(header, buffer)
		data.copy(buffer, 5)

		const crc = this.calcCRC(data)

		buffer[5 + data.length] = crc

		socket.write(buffer)
	}
}
